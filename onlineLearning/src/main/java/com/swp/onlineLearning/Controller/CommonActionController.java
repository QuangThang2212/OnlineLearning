...
public class CommonActionController {
    ...
    @GetMapping("/blog")
    public ResponseEntity<HashMap<String, Object>> getALlBlog(@RequestParam("page") int page, @RequestParam("limit") int limit, Principal principal) {
        String authority;
        if (principal == null) {
            authority = roleGuest;
        } else {
        ...
        HashMap<String, Object> json = blogService.findAllBlog(page, limit, authority);
    ...
    public ResponseEntity<HashMap<String, Object>> getBlogDetail(@RequestParam("id") String id, Principal principal) {
        String authority;
        if (principal == null) {
            authority = roleGuest;
        } else {
        ...
        HashMap<String, Object> json = blogService.getBlogDetail(id, authority);
        String type = json.get("type").toString();
        if (type.equals("true")) {
            return new ResponseEntity<>(json, HttpStatus.OK);
        } else {
